...
$no_dates_selected = $show_date_selector_link ? '<span class="epl_date_selector_use_message">' . epl__( "Please use the calendar above to select a date." ) . '</span>' : '';
...
foreach ( $date as $date_id => $field ):
    ...
    if ( $flow_mode == 'n' && epl_get_element( '_epl_event_status', $event_details ) != 3 && epl_get_date_timestamp( $event_details['_epl_start_date'][$date_id] ) < EPL_DATE )
        continue;
    ...
    <div id="<?php echo $date_id; ?>" class="epl_date_individual_date_wrapper">
